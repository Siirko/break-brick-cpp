...
class GameManager
{
  private:
    ...
    void generateBall(const float x, const float y, const int radius, std::mt19937 gen, float ball_speed);

  public:
    GameManager(int width, int height) { init(width, height); }
    void init(int width, int height, float ball_speed = 7.f, float paddle_speed = 0.5f);
